...
class TrialCostDataSource extends DataGridSource {
  final List<TrialCostItem> trialCost;
  final String controllerLevel;
  final String controllerSearch;
  final List<String> selectionItemsShow;

  TrialCostDataSource({
    required this.trialCost,
    required this.controllerLevel,
    required this.controllerSearch,
    required this.selectionItemsShow,
  }) {
    late List<TrialCostItem> trialCostFinallyList;
    if (controllerSearch.isEmpty && controllerLevel.isEmpty) {
      trialCostFinallyList = trialCost;
    } else if (controllerSearch.isNotEmpty && controllerLevel.isNotEmpty) {
      trialCostFinallyList = trialCost
          .where(
            (element) => element.acName!.contains(controllerSearch) && element.level! <= int.parse(controllerLevel),
          )
          .toList();
    } else if (controllerSearch.isNotEmpty) {
      trialCostFinallyList = trialCost
          .where(
            (element) => element.acName!.contains(controllerSearch),
          )
          .toList();
    } else if (controllerLevel.isNotEmpty) {
      trialCostFinallyList = trialCost.where((element) => element.level! <= int.parse(controllerLevel)).toList();
    }

    dataGridRows = trialCostFinallyList
  ...
  getMyString(TrialCostItem dataGridRow) {
    String levelSpace;
    int numberOfSpaces;
    for (int i = 1;; i++) {
      if (dataGridRow.level == i) {
        ...
        return "$levelSpace ${dataGridRow.acName}";
