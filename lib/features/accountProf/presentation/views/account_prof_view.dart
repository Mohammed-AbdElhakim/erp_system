...
import '../widgets/account_prof_view_body.dart';

class AccountProfView extends StatefulWidget {
  const AccountProfView({super.key, required this.pageData});

  final Pages pageData;
  static List<AllDropdownModel> myAllDropdownModelList = [];

  @override
  State<AccountProfView> createState() => _AccountProfViewState();
}

class _AccountProfViewState extends State<AccountProfView> {
  String? lang;
  ...
  Widget build(BuildContext context) {
    return ChangeStatusBarColor(
      child: Scaffold(
        ...
        body: BlocProvider(
          create: (context) =>
              GetAllDropdownListCubit(getIt.get<AccountProfRepoImpl>())
          ...
          child: BlocBuilder<GetAllDropdownListCubit, GetAllDropdownListState>(
            builder: (context, state) {
              if (state is GetAllDropdownListSuccess) {
                ...
                AccountProfView.myAllDropdownModelList = allDropdownModelList;
                return BlocProvider(
                  create: (context) =>
                      GetTableCubit(getIt.get<AccountProfRepoImpl>())
                        ..getTable(
