...
import 'package:erp_system/features/accountProf/presentation/widgets/pagination_widget.dart';
...
class AccountProfViewBody extends StatefulWidget {
  ...
  @override
  State<AccountProfViewBody> createState() => _AccountProfViewBodyState();
}

class _AccountProfViewBodyState extends State<AccountProfViewBody> {
  ...
  void initStatePage1() {
    ...
    myAllDropdownModelList = AccountProfView.myAllDropdownModelList;
  }
                          ...
                          ? CustomDropdown<String>.search(
                              ...
                              onChanged: (value) {
                                dropSetState(() {
                                  final ii = myListDrop!.firstWhere(
                                    (element) => element.text == value,
                                    orElse: () => ItemDrop(id: null, text: null), // تأكد إن فيه constructor آمن
                                  );
                                  if (ii.id != null) {
                                    widgetData['value'] = ii.id;
                                  } else {
                    ...
                    InkWell(
                      onTap: () {
                        dropSetState(() {
                          if (itemColumnList.isMulti == true) {
                            widgetData['value'] = [];
                            dropValueList.clear(); // << ضروري
                          } else {
                            widgetData['value'] = null;
                            dropValue = null; // << ضروري
                          }
                        });
                      },
                      child: const Padding(
    ...
    try {
      ...
      CustomAlertDialog.alertWithCustomContent(
        ...
        content: BlocProvider(
          create: (context) => AddEditCubit(getIt.get<AccountProfRepoImpl>()),
