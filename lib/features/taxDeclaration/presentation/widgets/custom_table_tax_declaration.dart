...
class CustomTableTaxDeclaration extends StatefulWidget {
  const CustomTableTaxDeclaration({
    super.key,
  ...
  @override
  State<CustomTableTaxDeclaration> createState() => _CustomTableTaxDeclarationState();
}

class _CustomTableTaxDeclarationState extends State<CustomTableTaxDeclaration> {
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    return DataGridRowAdapter(
      ...
      return Container(
        ...
        color: data[indexRow].toJson()["Color"] != null
            ? hexToColor(data[indexRow].toJson()["Color"])
            : null,
    ...
    if (columnList.insertType == "date") {
      try {
        String date = value.isNotEmpty
            ? DateFormat("yyyy-MM-dd", "en").format(DateTime.parse(value).toLocal())
            : '';
        return date;
      } catch (e) {
        String date = value.isNotEmpty
            ? DateFormat("yyyy-MM-dd", "en")
                .format(DateFormat("M/d/yyyy h:mm:ss a", "en").parse(value).toLocal())
            : '';
        return date;
      }
      // String date = value.isNotEmpty
