...
import '../../purchases/models/purchase_request_detail_model.dart';
import '../models/product_processpurchase_orders_model.dart';

typedef OnTapAdd<T> = void Function(T data);

class ProductProcessCustomIconButtonPurchaseOrders extends StatefulWidget {
  const ProductProcessCustomIconButtonPurchaseOrders({super.key, required this.onTapAdd});

  final OnTapAdd<List<Map<String, dynamic>>> onTapAdd;
  @override
  State<ProductProcessCustomIconButtonPurchaseOrders> createState() =>
      _ProductProcessCustomIconButtonPurchaseOrdersState();
}

class _ProductProcessCustomIconButtonPurchaseOrdersState
    extends State<ProductProcessCustomIconButtonPurchaseOrders> {
                  ...
                  child: BlocBuilder<DataGlobalCubit, DataGlobalState>(
                    builder: (context, state) {
                      if (state is DataGlobalSuccess) {
                        ProductProcessPurchaseOrdersModel purchaseOrdersModel =
                            ProductProcessPurchaseOrdersModel.fromJson(state.data);
                        return Column(
                          children: [
                            Expanded(
                                  ...
                                  for (var i in rowData) {
                                    ...
                                    for (var item in itemsList) {
                                      data.add({
                                        ...
                                        // "Expirdate": item.,
                                        "SizeID": item.sizeID ?? "",
                                        // "CostItemProccess": item.,
                                        // "contractBillId": item.,
                                        // "SecUnit": item.,
                                        "PQuntity": item.quntity ?? "",
                                        "PCost": item.cost ?? "",
                                        "Width": item.width ?? "",
                                        "length": item.length ?? "",
