...
import 'custom_table_add_edit.dart';

class AddSupplierMachine extends StatefulWidget {
  const AddSupplierMachine({super.key, this.tapData, required this.pageData, required this.listKey});

  final ListTaps? tapData;
  final Pages pageData;
  final List<dynamic> listKey;
  static List<dynamic> listMachineProcess = [];

  @override
  State<AddSupplierMachine> createState() => _AddSupplierMachineState();
}

class _AddSupplierMachineState extends State<AddSupplierMachine> {
  String? lang;
  ...
  void getDataList() async {
    try {
      ...
      Map<String, dynamic> machineProcess = await ApiService(Dio()).post(
        ...
        data: {"TableName": "MachinProcessView"},
        headers: {
          "Authorization": "Bearer $token",
          "CompanyKey": companyKey,
        },
      ...
      AddSupplierMachine.listMachineProcess = machineProcess['dynamicList'];
    } catch (e) {
      debugPrint(e.toString());
    }
  }
  ...
  List<Widget> getMyWidgetList({
    ...
    for (var item in listData) {
      ...
      //number
      else if (item.insertType == "number" && item.categoryTitle == categoryName && condition) {
        list.add(
          Padding(
            padding: const EdgeInsets.symmetric(vertical: 5),
            child: Column(
