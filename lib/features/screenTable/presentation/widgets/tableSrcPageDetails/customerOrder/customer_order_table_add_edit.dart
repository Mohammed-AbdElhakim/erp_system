...
import 'customer_order_alert_dialog_add_widget.dart';
import 'customer_order_alert_dialog_edit_widget.dart';

typedef OnTapAction<T> = void Function(T data);

class CustomerOrderTableAddEdit extends StatefulWidget {
  const CustomerOrderTableAddEdit({
    super.key,
...
class _CustomerOrderTableAddEditState extends State<CustomerOrderTableAddEdit> {
            ...
            IconButton(
              onPressed: () {
                showDialog(
                  ...
                  builder: (context) {
                    return AlertDialog(
                      content: CustomerOrderAlertDialogAddWidget(
                ...
                if (indexSelect.isNotEmpty) {
                  if (indexSelect.length > 1) {
                    ...
                    showDialog(
                      ...
                      builder: (context) {
                        return AlertDialog(
                          content: CustomerOrderAlertDialogEditWidget(
                            typeView: widget.typeView,
                            tapData: widget.tapData,
                            listKey: widget.listKey,
                            listHeader: widget.listHeader,
  ...
  void getDataPro(String type, String resultScanner) {
    if (type == "Add") {
      productId = AddCustomerOrder.listBarcodeData.firstWhere(
        (element) => element['BarcodeProc'] == resultScanner /*"11961"*/,
        orElse: () => {"ProductId": -1},
      )['ProductId'];

      double productPrice = AddCustomerOrder.listProductPrices.firstWhere(
            (element) =>
      ...
      if (productPrice != 0.0) {
        ...
        proName = AddCustomerOrder.listProduct.firstWhere(
              (element) => element["ProID"] == productId,
              orElse: () => {},
            )['ProName'] ??
            "";
      } else {
        double price = AddCustomerOrder.listProduct.firstWhere(
              (element) => element["ProID"] == productId,
              orElse: () => {},
            )['ProPrice'] ??
            -1;
