...
import 'add_supply_order.dart';
import 'edit_supply_order.dart';
import 'supply_order_alert_dialog_add_widget.dart';
import 'supply_order_alert_dialog_edit_widget.dart';
import 'widgets/supply_order_custom_icon_button_purchase_orders.dart';

typedef OnTapAction<T> = void Function(T data);

class SupplyOrderTableAddEdit extends StatefulWidget {
  const SupplyOrderTableAddEdit({
    super.key,
  ...
  @override
  State<SupplyOrderTableAddEdit> createState() => _SupplyOrderTableAddEditState();
}

class _SupplyOrderTableAddEditState extends State<SupplyOrderTableAddEdit> {
      ...
      children: [
        Row(
          children: [
            IconButton(
              onPressed: () {
                showDialog(
                  ...
                  builder: (context) {
                    return AlertDialog(
                      content: SupplyOrderAlertDialogAddWidget(
                ...
                if (indexSelect != -1) {
                  showDialog(
                    ...
                    builder: (context) {
                      ...
                      return AlertDialog(
                        content: SupplyOrderAlertDialogEditWidget(
                          typeView: widget.typeView,
                          tapData: widget.tapData,
                          listKey: widget.listKey,
                          listHeader: widget.listHeader,
            ...
            SupplyOrderCustomIconButtonPurchaseOrders(
  ...
  buildMyWidget(ItemListSetupModel columnList, int indexRow) {
    ...
    switch (columnList.insertType) {
      ...
      case "dropdown":
        ...
        // if (widget.typeView == "Add") {
        //   for (var element in AddSupplyOrder.listProduct) {
        //     if (element['ProID'].toString() == data) {
        //       val = element['ProName'] ?? "";
        //     }
        //   }
        // } else {
        //   for (var element in EditSupplyOrder.listProduct) {
        //     if (element['ProID'].toString() == data) {
        //       val = element['ProName'] ?? "";
        //     }
        //   }
  ...
  void getDataPro(String type, String resultScanner) {
    if (type == "Add") {
      productId = AddSupplyOrder.listBarcodeData.firstWhere(
        (element) => element['BarcodeProc'] == resultScanner /*"11961"*/,
        orElse: () => {"ProductId": -1},
      )['ProductId'];

      double price = AddSupplyOrder.listProduct.firstWhere(
      ...
      if (price != -1) {
        proPrice = price;
        proName = AddSupplyOrder.listProduct.firstWhere(
              (element) => element["ProID"] == productId,
              orElse: () => {},
            )['ProName'] ??
            "";
      ...
      productId = EditSupplyOrder.listBarcodeData.firstWhere(
        (element) => element['BarcodeProc'] == resultScanner,
        orElse: () => {"ProductId": -1},
      )['ProductId'];

      double price = EditSupplyOrder.listProduct.firstWhere(
            (element) => element["ProID"] == productId,
            orElse: () => {},
          )['ProPrice'] ??
          -1;
      if (price != -1) {
        proPrice = price;
        proName = EditSupplyOrder.listProduct.firstWhere(
              (element) => element["ProID"] == productId,
              orElse: () => {},
            )['ProName'] ??
            "";
