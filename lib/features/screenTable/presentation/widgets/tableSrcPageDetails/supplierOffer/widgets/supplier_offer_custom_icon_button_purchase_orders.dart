...
import '../models/supplier_offer_purchase_orders_model.dart';
import '../models/supplier_offer_purchase_request_detail_model.dart';

typedef OnTapAdd<T> = void Function(T data);

class SupplierOfferCustomIconButtonPurchaseOrders extends StatefulWidget {
  const SupplierOfferCustomIconButtonPurchaseOrders({
    super.key,
    required this.onTapAdd,
  });
  final OnTapAdd<List<Map<String, dynamic>>> onTapAdd;
  @override
  State<SupplierOfferCustomIconButtonPurchaseOrders> createState() =>
      _SupplierOfferCustomIconButtonPurchaseOrdersState();
}

class _SupplierOfferCustomIconButtonPurchaseOrdersState
    extends State<SupplierOfferCustomIconButtonPurchaseOrders> {
  late SupplierOfferPurchaseRequestDetailModel purchaseRequestDetailModel;
                  ...
                  child: BlocBuilder<DataGlobalCubit, DataGlobalState>(
                    builder: (context, state) {
                      if (state is DataGlobalSuccess) {
                        SupplierOfferPurchaseOrdersModel purchaseOrdersModel =
                            SupplierOfferPurchaseOrdersModel.fromJson(state.data);
                        return BlocProvider(
                          create: (context) =>
                              DataGlobalCubit(getIt.get<GeneralRepoImpl>())
                                ..getDataGlobal(bodyRequest: {
                                  "company": false,
                          ...
                          child: BlocBuilder<DataGlobalCubit, DataGlobalState>(
                            builder: (context, sstate) {
                              if (sstate is DataGlobalSuccess) {
                                ProductViewModel productViewModel =
                                    ProductViewModel.fromJson(sstate.data);
                                return Column(
                                  children: [
                                    Expanded(
                                      child: BuildCustomTable(
                                        purchaseOrdersList:
                                            purchaseOrdersModel.dynamicList ?? [],
                                        onTapRow: (rowData) {
                                          data.clear();
                                          for (var i in rowData) {
                                            ...
                                            for (var item in itemsList) {
                                              double price =
                                                  productViewModel.dynamicListProduct!
                                                          .firstWhere(
                                                            (element) =>
                                                                element.proID ==
                                                                item.productId,
                                              ...
                                              data.add({
                                                ...
                                                "PricePerUnit": price,
                                                "DetailName": item.detailItemID,
                                                "Quantity": item.quntity,
                                                // "Discount": item.,
                                                "TotalPrice": item.totalCost,
                                                "UnitCost": item.unitID,
                                                // "GrandTotal": item.,
                                              });
                                            }
                                          }
                              ...
                              } else if (sstate is DataGlobalFailure) {
                                return CustomErrorMassage(
                                    errorMassage: sstate.errorMassage);
                              } else if (sstate is DataGlobalLoading) {
    ...
    try {
      ...
      setState(() {
        purchaseRequestDetailModel =
            SupplierOfferPurchaseRequestDetailModel.fromJson(purchaseRequestDetail);
      });
    } catch (e) {
