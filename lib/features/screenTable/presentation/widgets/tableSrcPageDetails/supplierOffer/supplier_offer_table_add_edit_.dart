...
import 'add_supplier_offer.dart';
...
class SupplierOfferTableAddEdit extends StatefulWidget {
  const SupplierOfferTableAddEdit({
    super.key,
  ...
  @override
  State<SupplierOfferTableAddEdit> createState() => _SupplierOfferTableAddEditState();
}

class _SupplierOfferTableAddEditState extends State<SupplierOfferTableAddEdit> {
            ...
            IconButton(
              onPressed: () {
                showDialog(
                  ...
                  builder: (context) {
                    return AlertDialog(
                      content: SupplierOfferAlertDialogAddWidget(
                ...
                if (indexSelect != -1) {
                  showDialog(
                    ...
                    builder: (context) {
                      ...
                      return AlertDialog(
                        content: SupplierOfferAlertDialogEditWidget(
                          typeView: widget.typeView,
                          tapData: widget.tapData,
                          listKey: widget.listKey,
                          listHeader: widget.listHeader,
  ...
  void getDataPro(String type, String resultScanner) {
    if (type == "Add") {
      productId = AddSupplierOffer.listBarcodeData.firstWhere(
        (element) => element['BarcodeProc'] == resultScanner /*"11961"*/,
        orElse: () => {"ProductId": -1},
      )['ProductId'];

      double price = AddSupplierOffer.listProduct.firstWhere(
      ...
      if (price != -1) {
        proPrice = price;
        proName = AddSupplierOffer.listProduct.firstWhere(
              (element) => element["ProID"] == productId,
              orElse: () => {},
            )['ProName'] ??
            "";
      ...
      productId = EditSupplierOffer.listBarcodeData.firstWhere(
        (element) => element['BarcodeProc'] == resultScanner,
        orElse: () => {"ProductId": -1},
      )['ProductId'];

      double price = EditSupplierOffer.listProduct.firstWhere(
            (element) => element["ProID"] == productId,
            orElse: () => {},
          )['ProPrice'] ??
          -1;
      if (price != -1) {
        proPrice = price;
        proName = EditSupplierOffer.listProduct.firstWhere(
              (element) => element["ProID"] == productId,
              orElse: () => {},
            )['ProName'] ??
            "";
