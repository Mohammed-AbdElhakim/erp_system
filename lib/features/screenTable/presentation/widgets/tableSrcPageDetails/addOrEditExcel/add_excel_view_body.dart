...
class AddExcelViewBody extends StatefulWidget {
  const AddExcelViewBody({super.key, required this.pageData, required this.listKey, this.tapData});
  ...
  List<Widget> getMyWidgetList({
    ...
    for (var item in listData) {
      ...
      if (item.insertType == "date" && item.categoryTitle == categoryName && condition) {
        String date = singleObject[item.columnName] == null
            ? ""
            : DateFormat("yyyy-MM-dd", 'en').format(DateTime.parse(singleObject[item.columnName]!.toString()));
        list.add(
          Padding(
            padding: const EdgeInsets.symmetric(vertical: 5),
            child: Column(
      ...
      //time
      if (item.insertType == "time" && item.insertVisable == true && item.categoryTitle == categoryName && condition) {
          ...
          onTimeSelected: (timeSelect) {
            if (timeSelect != null) {
              setState(() {
                ...
                singleObject[item.columnName!.toString()] = timeSelect;
              });
            }
          },
          onClear: () {
