...
import 'product_card_repo.dart';

class ProductCardRepoImpl implements ProductCardRepo {
  final ApiService apiService;

  ProductCardRepoImpl(this.apiService);
  ...
  @override
  Future<Either<Failure, ProductCardModel>> getTableProductCard({required Map<String, dynamic> objectData}) async {
    try {
      String companyKey = await Pref.getStringFromPref(key: AppStrings.companyIdentifierKey) ?? "";
      String token = await Pref.getStringFromPref(key: AppStrings.tokenKey) ?? "";
      Map<String, dynamic> data = await apiService.post(
        endPoint: "web/Productcard/getDataGlobal",
        data: objectData,
        headers: {
          "Authorization": "Bearer $token",
          "CompanyKey": companyKey,
      ...
      ProductCardModel productCardModel = ProductCardModel.fromJson(data);
